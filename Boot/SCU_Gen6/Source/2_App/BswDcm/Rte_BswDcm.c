...
void REoiBswDcm_pDcmCmd_processDiagRequest(uint8 isPhysical, P_uint8 diagReq, uint8 length)
{
    ...
    frameType = diagReq[0] >> 4u; // single frame + sf_dl ==> 1byte 4bit를 밀어서 프레임이 무슨 프레임인지만 보겠다.
    if (frameType == FRAME_TYPE_SINGLE)
    {
        stBswDcmData.isPhysical = isPhysical;
        stBswDcmData.diagDataLengthRx = (diagReq[0] & 0x0Fu); //  sfdl쪽이 길이
    ...
    else if (frameType == FRAME_TYPE_FIRST)
    {
        ...
        (void)memcpy(stBswDcmData.diagDataBufferRx, &diagReq[2], stBswDcmData.diagDataLengthRx); // frame type이랑 길이정보를 뺀 데이터를 버퍼에 저장
        // Send FlowControl
...
static uint8 routineControl(void)
{
	...
	{
	    routineControlOptionRecord = ((uint16_t)stBswDcmData.diagDataBufferRx[4] << 8) | (uint16_t)stBswDcmData.diagDataBufferRx[5]; //RID는 2Byte이기 때문에 2byte파싱
	    if(routineControlOptionRecord == RCOR_EraseMemory_App)
	    {
	        Rte_Call_BswDcm_rEcuAbsFls_erasePflashBlock(PFLASH_AREA_APPICATION_PRIMARY, &errorResult); // PRIMARY 영역을 지워라
	    }
	    else
	    {
	        errorResult = NRC_RequestOutRange;
