...
typedef struct {
        ...
        SHA256_INFO signInfo;
        uint8 signKey[MEMORY_SIZE_SIGN_KEY];
        uint8 signResult[MEMORY_SIZE_SIGNATURE];
...
void REcbBswDcm_initialize(void)
{
    ...
    memset(stBswDcmData.signKey, 0x11, MEMORY_SIZE_SIGN_KEY);
...
static uint8 routineControl(void)
{
    ...
    uint16 reprogrammingSuccessCounter;
    ...
    {
        //Rte_Call_BswDcm_rEcuAbsFls_copyPflashSector(PFLASH_AREA_APPICATION_PRIMARY, PFLASH_AREA_APPICATION_SECONDARY, &errorResult);
        //SHA256_Init(&stBswDcmData.signInfo);
        //SHA256_Process(&stBswDcmData.signInfo, stBswDcmData.signKey, MEMORY_SIZE_SIGN_KEY);
        //SHA256_Process(&stBswDcmData.signInfo, (void*)(MEMORY_ADDRESS_APPLICATION+MEMORY_SECONDARY_OFFSET),//80180000
        //       MEMORY_SIZE_APPLICATION-MEMORY_SIZE_SIGNATURE);//7FFE0

        //SHA256_Close(&stBswDcmData.signInfo, stBswDcmData.signResult);
        //signResult vs 80100000 + 524288 + (524288 - 32) - 32 = 801F FFC0
        if(memcmp(stBswDcmData.signResult, (void*)(MEMORY_ADDRESS_APPLICATION+MEMORY_SECONDARY_OFFSET+MEMORY_SIZE_APPLICATION-MEMORY_SIZE_SIGNATURE), MEMORY_SIZE_SIGNATURE) == 0) {
            Rte_Call_BswDcm_rEcuAbsFls_copyPflashSector(PFLASH_AREA_APPICATION_PRIMARY, PFLASH_AREA_APPICATION_SECONDARY, &errorResult);
            ...
            {
                Rte_Call_BswDcm_rEcuAbsNvm_readNvmData(DATA_ID_ReprogrammingSuccessCounter, (P_uint8)&reprogrammingSuccessCounter);
                reprogrammingSuccessCounter++;
                Rte_Call_BswDcm_rEcuAbsNvm_writeNvmData(DATA_ID_ReprogrammingSuccessCounter, (P_uint8)&reprogrammingSuccessCounter);
            }
        }
        else {
            errorResult = NRC_GeneralProgrammingFailure;
...
static uint8 requestDownload(void)
{
    ...
    uint32 memorySize;
    uint16 reprogrammingAttemptCounter = 0u;
    uint16 reprogrammingSuccessCounter = 0u;
    ...
    else
    {
        ...
        Rte_Call_BswDcm_rEcuAbsNvm_readNvmData(DATA_ID_ReprogrammingAttemptCounter, (P_uint8)&reprogrammingAttemptCounter);
        Rte_Call_BswDcm_rEcuAbsNvm_readNvmData(DATA_ID_ReprogrammingSuccessCounter, (P_uint8)&reprogrammingSuccessCounter);
        reprogrammingAttemptCounter++;
        if((reprogrammingAttemptCounter - reprogrammingSuccessCounter)> 3) {
            errorResult = NRC_GeneralProgrammingFailure;
        }
        Rte_Call_BswDcm_rEcuAbsNvm_writeNvmData(DATA_ID_ReprogrammingAttemptCounter,  (P_uint8)&reprogrammingAttemptCounter);
...
static uint8 transferData(void)
{
    ...
    else
    {
        ...
        {
            if((stBswDcmData.downloadDataLength + i) >= (stBswDcmData.downloadMemorySize - MEMORY_SIZE_SIGNATURE))
            {
                break;
            }
            stBswDcmData.diagDataBufferRx[i+2] -= 5;
        }
        //제어기에 requestdownload memory address는 무조건 0x80100000임.->  0x80100000 + 524288 = 0x80180000 -> 백업영역 주소
        Rte_Call_BswDcm_rEcuAbsFls_writePflash(stBswDcmData.downloadMemoryAddress + stBswDcmData.downloadDataLength + MEMORY_SECONDARY_OFFSET, dataLength, &stBswDcmData.diagDataBufferRx[2], &errorResult);
        if(errorResult == FALSE)
        {
            ...
            if(stBswDcmData.downloadDataLength != stBswDcmData.downloadMemorySize) {
                SHA256_Process(&stBswDcmData.signInfo, &stBswDcmData.diagDataBufferRx[2], dataLength);
            }
            else {
                SHA256_Process(&stBswDcmData.signInfo, &stBswDcmData.diagDataBufferRx[2], dataLength-MEMORY_SIZE_SIGNATURE);
            }
...
static uint8 requestTransferExit(void)
{
    ...
    {
        //0x801fffe0 -> 백업영역의 패턴 주소
        Rte_Call_BswDcm_rEcuAbsFls_writePflash(MEMORY_PATTERN_VALID_APP_ADDR + MEMORY_SECONDARY_OFFSET, MEMORY_DATA_VALID_PATTERN_LENGTH, (P_uint8) MEMORY_DATA_VALID_PATTERN_VALUE, &errorResult);
        if(errorResult == FALSE)
        {
