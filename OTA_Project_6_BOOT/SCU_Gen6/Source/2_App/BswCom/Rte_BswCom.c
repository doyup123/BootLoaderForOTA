...
void REcbBswCom_canRxIndication(void) /*intelligent can 툴과 CAN DATA를 주고받는 코드*/
{
    uint8 handle;
    uint8 length;
    uint32 id;
    ...
    else if(handle == RX_MSG_IDX_FUN_REQ) /*canid : 7df, can idx : 1*/
    {
        Rte_Call_BswCom_rDcmCmd_processDiagRequest(FALSE, data, length);
    }
    else if(handle == RX_MSG_IDX_PHY_REQ) /*canid : 7d3, can idx : 0*/
    {
