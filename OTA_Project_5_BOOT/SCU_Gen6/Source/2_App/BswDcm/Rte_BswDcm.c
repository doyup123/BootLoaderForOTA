...
#define MEMORY_SIZE_SIGNATURE 32u
#define MEMORY_SIZE_KEY 32u

#define CommunicationType_normalCommunicationMessages		0x01u
#define MEMORY_SECONDARY_OFFSET (512*1024)
...
void REcbBswDcm_initialize (void)
{
    ...
    (void)memset(stBswDcmData.Key, 0x11, MEMORY_SIZE_KEY);
}
...
static uint8 routineControl (void)
{
    ...
    {
        routineControlOptionRecord = ((uint16_t) stBswDcmData.diagDataBufferRx[4] << 8)
                | (uint16_t) stBswDcmData.diagDataBufferRx[5];
    ...
    {
                /*SHA256_Init(&stBswDcmData.signInfo); request download로 이동
                SHA256_Process(&stBswDcmData.signInfo, stBswDcmData.Key, MEMORY_SIZE_KEY);
                SHA256_Process(&stBswDcmData.signInfo, (void*) (MEMORY_ADDRESS_APPLICATION + MEMORY_SECONDARY_OFFSET),
                MEMORY_SIZE_APPLICATION - MEMORY_SIZE_SIGNATURE);
                SHA256_Close(&stBswDcmData.signInfo, stBswDcmData.signResult);*/


                if (memcmp(stBswDcmData.signResult,
                        (void*) (MEMORY_ADDRESS_APPLICATION + MEMORY_SECONDARY_OFFSET + MEMORY_SIZE_APPLICATION - MEMORY_SIZE_SIGNATURE)
                        , MEMORY_SIZE_SIGNATURE) == 0)
                {
                    Rte_Call_BswDcm_rEcuAbsFls_copyPflashSector(PFLASH_AREA_APPICATION_PRIMARY,
                            PFLASH_AREA_APPICATION_SECONDARY, &errorResult);
...
static uint8 requestDownload (void)
{
    ...
    else
    {
        ...
        memoryAddress = ((uint32) stBswDcmData.diagDataBufferRx[3] << 24u)
                | ((uint32) stBswDcmData.diagDataBufferRx[4] << 16u) | ((uint32) stBswDcmData.diagDataBufferRx[5] << 8u)
                | ((uint32) stBswDcmData.diagDataBufferRx[6] << 0u);
        memorySize = ((uint32) stBswDcmData.diagDataBufferRx[7] << 24u)
                | ((uint32) stBswDcmData.diagDataBufferRx[8] << 16u) | ((uint32) stBswDcmData.diagDataBufferRx[9] << 8u)
                | ((uint32) stBswDcmData.diagDataBufferRx[10] << 0u);
...
static uint8 transferData (void)
{
    ...
    else
    {
        ...
        Rte_Call_BswDcm_rEcuAbsFls_writePflash(stBswDcmData.downloadMemoryAddress + stBswDcmData.downloadDataLength +
            MEMORY_SECONDARY_OFFSET, dataLength, &stBswDcmData.diagDataBufferRx[2], &errorResult);
        if (errorResult == FALSE)
        {
...
static uint8 requestTransferExit (void)
{
    ...
    {
        Rte_Call_BswDcm_rEcuAbsFls_writePflash(MEMORY_PATTERN_VALID_APP_ADDR + MEMORY_SECONDARY_OFFSET,
            MEMORY_DATA_VALID_PATTERN_LENGTH, (P_uint8) MEMORY_DATA_VALID_PATTERN_VALUE, &errorResult);
        if (errorResult == FALSE)
        {
