...
void EA_erasePflashBlock (uint8 pflashArea, P_uint8 errorResult) //지우기
{
    Mcal_DisableAllInterrupts();
    EA_wdgTriggerUpdateSlow();
    if (pflashArea == PFLASH_AREA_APPICATION_PRIMARY)
    {
        *errorResult = (uint8_t) FlsLoader_Erase(FLSLOADER_PFLASH0_START_ADDRESS + FLSLOADER_PF_S23, 2u); //엡영역
    }
