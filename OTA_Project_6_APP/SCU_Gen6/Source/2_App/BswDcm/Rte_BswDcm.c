...
//enum { /* DIAGNOSTIC_SESSION */
//    DIAG_SESSION_DEFAULT,
//    DIAG_SESSION_EXTENDED
//};

#define DID_ReprogrammingAttemptCounter     0xF102u
#define DID_ReprogrammingSuccessCounter     0xF103u
...
void REoiBswDcm_pDcmCmd_processDiagRequest(uint8 isPhysical, P_uint8 diagReq, uint8 length) {
    (void) length;
    uint8_t frameType = diagReq[0] >> 4u; //첫번째 바이트의 상위 4비트 : 프레임 타입, 하위 4비트 : 데이터 길이 오른쪽으로 4번 밀어서 프레임타입만 확인.
...
static uint8 securityAccess(void) {
    ...
    if(stBswDcmData.stDiagStatus.currentSession == DIAG_SESSION_DEFAULT) {/*default session 지원안함.*/
        errorResult = NRC_ServiceNotSupportedInActiveSession;/*0x7F*/
    } else if(subFunctionId == SFID_SecurityAccess_RequestSeed) {/*0x01*/
        if(stBswDcmData.diagDataLengthRx != 2u) {
            errorResult = NRC_IncorrectMessageLengthOrInvalidFormat;/*0x13*/
        }
        else
        {
        ...
        if(stBswDcmData.diagDataLengthRx != 4u) {
            ...
            if(key == 0x5678u) {
                stBswDcmData.diagDataLengthTx = 2u;
                stBswDcmData.diagDataBufferTx[0] = (FRAME_TYPE_SINGLE << 4u) |
                        stBswDcmData.diagDataLengthTx;
                stBswDcmData.diagDataBufferTx[1] = SID_SecurityAccess + 0x40u;
                stBswDcmData.diagDataBufferTx[2] = subFunctionId;
...
static uint8 writeDataByIdentifier(void) {
    ...
    uint16 reprogrammingAttemptCounter;
    ...
    else if(dataIdentifier == DID_ReprogrammingAttemptCounter) {
        if(stBswDcmData.diagDataLengthRx != 5u) {
            errorResult = NRC_IncorrectMessageLengthOrInvalidFormat;
        }
        else {
            reprogrammingAttemptCounter = (uint16)stBswDcmData.diagDataBufferRx[3] << 8 | stBswDcmData.diagDataBufferRx[4];
    ...
    else if(dataIdentifier == DID_ReprogrammingSuccessCounter) {
        if(stBswDcmData.diagDataLengthRx != 5u) {
            errorResult = NRC_IncorrectMessageLengthOrInvalidFormat;
        }
        else {
            reprogrammingSuccessCounter = (uint16)stBswDcmData.diagDataBufferRx[3] << 8 | stBswDcmData.diagDataBufferRx[4];
...
static uint8 readDataByIdentifier(void) {
    ...
    if(stBswDcmData.diagDataLengthRx != 3u) {
        ...
        stBswDcmData.diagDataLengthTx = 5u;
        stBswDcmData.diagDataBufferTx[0] = (FRAME_TYPE_SINGLE << 4u) | stBswDcmData.diagDataLengthTx;
        stBswDcmData.diagDataBufferTx[1] = SID_ReadDataByIdentifier + 0x40u;
        stBswDcmData.diagDataBufferTx[2] = (uint8)((dataIdentifier & 0xFF00) >> 8u);
        stBswDcmData.diagDataBufferTx[3] = (uint8)(dataIdentifier & 0xFFu);
        stBswDcmData.diagDataBufferTx[4] = (uint8)(reprogrammingAttemptCounter >> 8u);
        stBswDcmData.diagDataBufferTx[5] = (uint8)reprogrammingAttemptCounter;
    }
    else if(dataIdentifier == DID_ReprogrammingSuccessCounter) {
        ...
        stBswDcmData.diagDataBufferTx[2] = (uint8)((dataIdentifier & 0xFF00) >> 8u);
        stBswDcmData.diagDataBufferTx[3] = (uint8)(dataIdentifier & 0xFFu);
        stBswDcmData.diagDataBufferTx[4] = (uint8)(reprogrammingSuccessCounter >> 8u);
        stBswDcmData.diagDataBufferTx[5] = (uint8)reprogrammingSuccessCounter;
    }
    else {
        errorResult = NRC_RequestOutRange;
    }
